...
final private class InMemoryTodoRepository(
  ...
  override def update(
    ...
    for {
      oldValue <- getById(id)
      result   <- oldValue.fold[UIO[Option[TodoItem]]](ZIO.none) { x =>
                    val newValue = x.update(todoItemForm)
                    ref.update(store => store + (newValue.id -> newValue)).as(Some(newValue))
                  }
    } yield result
}
...
object InMemoryTodoRepository {
  ...
  val layer: ZLayer[Any, Nothing, TodoRepository] =
    (for {
      ...
      counter <- Ref.make(0L)
    } yield new InMemoryTodoRepository(ref, counter)).toLayer
}
